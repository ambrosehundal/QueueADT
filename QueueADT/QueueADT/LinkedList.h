...
public:
	...
	template <class T>
	friend ostream& operator<< (ostream &output, const LinkedList<T>* Qptr);
};
...
/******************************************
* bool addNode();
* adds a node entry to the linkedlist 
  This function is specifically for QueueADT
*/
template <class T>
bool LinkedList<T>::addNode(T info) {
	Node<T>* temp1 = new Node<T>(info);
